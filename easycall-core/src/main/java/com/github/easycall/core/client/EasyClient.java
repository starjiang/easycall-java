...
public final class EasyClient implements ClientMessageDispatcher {
    ...
    public CompletableFuture<EasyPackage> asyncCFRequest(String service,String method,Object body, int timeout){
        EasyHead head = EasyHead.newInstance();
        head.setService(service).setMethod(method);
        return asyncCFRequest(EasyPackage.FORMAT_MSGPACK,head,body,timeout);
    }

    public CompletableFuture<EasyPackage> asyncCFRequest(int format,EasyHead head,Object body, int timeout,String ip,int port){
        ResponseFuture future = asyncRequest(format,head,body,timeout,ip,port);
        CompletableFuture<EasyPackage> completableFuture = new CompletableFuture<>();

        future.setCallback((future1)->{
            if (future1.isException()){
                completableFuture.completeExceptionally(future1.getException());
            }else{
                completableFuture.complete(future1.getResult());
            }
        });
